...
The `pvflux` package handles timezones automatically and consistently:

1. **Input flexibility**: Timestamps can be provided as POSIXct, POSIXlt, character, or numeric (seconds since epoch)
2. **Automatic UTC conversion**: All internal solar position calculations are performed in UTC
...
This means you can work with local time data without manual conversion:

```{r timezone_example, eval=FALSE}
# Using local South African time (SAST = UTC+2)
time_local <- seq(as.POSIXct("2026-01-15 00:00", tz = "Africa/Johannesburg"),
                  length.out = 24, by = "hour")

# The package converts to UTC internally, computes solar position,
# then returns results in the original "Africa/Johannesburg" timezone
result_local <- erbs_decomposition(
  time = time_local,
  lat = -30.6279,
  lon = 24.0054,
...
# Output timestamps preserve the original timezone
attr(result_local$time, "tzone")  # "Africa/Johannesburg"
```

For consistency with weather data sources (which typically use UTC), we recommend using UTC timestamps:
